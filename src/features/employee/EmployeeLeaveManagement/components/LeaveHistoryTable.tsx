...
interface LeaveHistoryTableProps {
  leaveRequests: Leave[]; // Array of leave request data
  ...
  const columns = [
    {
      ...
      dataIndex: 'leave_type', // Data index for leave type
      key: 'leave_type', // Unique key for the column
    ...
    },
    {
      ...
      render: (record: Leave) => ( // Custom render function for dates
        <Space direction="vertical" size={0}>
          <span>{new Date(record.start_date).toLocaleDateString()}</span> {/* Display start date */}
          <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
            to {new Date(record.end_date).toLocaleDateString()} {/* Display end date */}
          </span>
        </Space>
      )
    },
    ...
    },
    {
      ...
      render: (record: Leave) => ( // Custom render function for recipients
        <Avatar.Group maxCount={3} size="small"> {/* Avatar group for multiple recipients */}
          <Tooltip title="HR Manager"> {/* Tooltip for recipient */}
            <Avatar size="small" icon={<User size={12} />} /> {/* Avatar for recipient */}
          </Tooltip>
        </Avatar.Group>
      )
    },
    {
      ...
      dataIndex: 'created_at', // Data index for applied date
      key: 'created_at', // Unique key for the column
    ...
    },
    {
      ...
      render: (record: Leave) => ( // Custom render function for actions
        <Space>
          <Button size="small" icon={<Download size={14} />} /> {/* Download button */}
          <Tooltip title="View Details"> {/* Tooltip for details */}
